...
macro_rules! compare_with_gnu {
    ( $ts:expr, $args:expr ) => {{
        ...
        #[cfg(any(target_os = "linux", target_os = "android"))]
        {
            let reference = expected_result(&$ts, $args).unwrap();
...
mod test_raw_string_parser {
    ...
    fn test_multi_byte_codes_take_one_look_at_correct_data_and_end_behavior() {
        ...
        for _i in 0..3 {
            ...
            uut.take_one().unwrap();
            assert_eq!(uut.look_at().unwrap(), b'x');
            uut.take_one().unwrap();
        }
        assert_eq!(uut.look_at().unwrap(), first_byte_of_owl);
        uut.take_one().unwrap();
    ...
    fn test_multi_byte_codes_skip_one_only_allowed_for_ascii_chars_skip_until_char_skip_until_end()
    {
        ...
        uut.skip_until_ascii_char_or_end(b'x').unwrap(); // skip 🦉🦉🦉🦉 till end
    ...
    fn test_multi_byte_codes_skip_multiple_ascii_bounded_good_and_bad() {
        ...
        uut.skip_multiple_ascii_bounded(0).unwrap();
        assert_eq!(uut.get_look_at_pos(), 0);
        for i in 1..12 {
            uut.skip_multiple_ascii_bounded(i).unwrap_err();
            assert_eq!(uut.get_look_at_pos(), 0);
        }
        uut.skip_multiple_ascii_bounded(12).unwrap(); // skips 🦉🦉🦉
        assert_eq!(uut.get_look_at_pos(), 12);

        uut.take_one().unwrap(); // take x
        assert_eq!(uut.get_look_at_pos(), 13);
        uut.skip_multiple_ascii_bounded(12).unwrap_err();
        assert_eq!(uut.get_look_at_pos(), 13);
        uut.skip_multiple_ascii_bounded(13).unwrap(); // skips 🦉🦉x🦉
        assert_eq!(uut.get_look_at_pos(), 26);
        uut.take_one().unwrap(); // take x

        uut.skip_multiple_ascii_bounded(15).unwrap_err();
        assert_eq!(uut.get_look_at_pos(), 27);
        uut.skip_multiple_ascii_bounded(17).unwrap_err();
        assert_eq!(uut.get_look_at_pos(), 27);
        uut.skip_multiple_ascii_bounded(16).unwrap(); // skips 🦉🦉🦉🦉
        assert_eq!(uut.get_look_at_pos(), 43);
    ...
    fn test_multi_byte_codes_put_string_utf8_start_middle_end() {
        ...
        uut.put_string_utf8("oo🦔").unwrap();
        uut.take_one().unwrap(); // take x
        uut.skip_until_ascii_char_or_end(b'\n').unwrap(); // skips till end
    ...
    fn test_multi_byte_codes_look_at_remaining_start_middle_end() {
        ...
        assert_eq!(uut.look_at_remaining().unwrap(), input);
        uut.take_one().unwrap(); // takes 🦉🦉🦉
        assert_eq!(uut.look_at_remaining().unwrap(), input.get(12..).unwrap());
        uut.skip_until_ascii_char_or_end(b'\n').unwrap(); // skips till end
        assert_eq!(uut.look_at_remaining().unwrap(), "");
