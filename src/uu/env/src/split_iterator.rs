...
impl<'a> SplitIterator<'a> {
    ...
    fn push_word_to_words(&mut self) -> Result<(), ParseError> {
        let byte_word = self.raw_parser.take_collected_output()?;
    ...
    fn parse_braced_variable_name(&mut self) -> Result<(&'a str, Option<&'a str>), ParseError> {
        let pos_start = self.raw_parser.get_look_at_pos();
        ...
        loop {
            match self.get_current_char() {
                None => {
                    return Err(ParseError::ParsingOfVariableNameFailed {
                        pos: self.raw_parser.get_look_at_pos(), msg: "Missing closing brace".into() })
                },
                Some(c) if !c.is_ascii() || c.is_ascii_alphanumeric() || c == b'_' => {
                    self.skip_one()?;
                }
                Some(b':') => {
                    varname_end = self.raw_parser.get_look_at_pos();
                    loop {
                        match self.get_current_char() {
                            None => {
                                return Err(ParseError::ParsingOfVariableNameFailed {
                            ...
                            Some(b'}') => {
                                default_end = Some(self.raw_parser.get_look_at_pos());
                                self.skip_one()?;
                                break
                            },
                            Some(_) => {
        ...
        let default = if let Some(default_end) = default_end {
            Some(self.raw_parser.get_substring(&Range {
                start: varname_end + 1,
                end: default_end,
            })?)
        } else {
        ...
        let varname = self.raw_parser.get_substring(&Range {
            start: pos_start,
            end: varname_end,
        })?;
        ...
        loop {
            ...
            self.state = match self.state {
                ...
                Comment => match c {
                    ...
                    Some(_) => {
                        self.raw_parser.skip_until_ascii_char_or_end(b'\n')?;
                        Comment
                    }
                },
            };
